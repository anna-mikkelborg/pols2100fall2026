...
Let's subset `officerdata` to look at the data specifically for female officers.
...
## Now let's subset to look at data on *stops conducted by female officers only.*

(hint: you can subset on more than one variable!)
...
Let's compare the rate of contraband found among stops by male and female officers.
...
```{webr}
female.officer.stops <- subset(officerdata, officerdata$officer_female == 1 & search_occur == 1)
